function [outX, optimflag, maxR2, optimplot] = quick_glm_grid_noL(Ymeas,pa,snum,dbug,mnum)
...
for gs = 1:pa.gnum
    if mnum == 1
        tempcost(gs) = quick_glm_cost([pa.models(mnum).B(gs,:) pa.locs 0 round(pa.dectime(snum)) pa.models(mnum).tmax(gs) pa.models(mnum).yint(gs)],Ymeas);
    end
end
...
X0 = [B0 repmat(Blocs0,pa.ssnum,1) zeros(pa.ssnum,1) (ones(pa.ssnum,1)*round(pa.dectime(snum))) pa.models(mnum).tmax(spind) pa.models(mnum).yint(spind)];
...
%     end
    [tempout(ss,:),cost,optimflag(ss)] = quick_glm_optim_noL(Ymeas,X0(ss,:));
    R2(ss) = 1 - (cost/SSt);
    if dbug == 2
        if ss == 1
