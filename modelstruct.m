...
ff = fullfact([length(beta) length(loc) length(yint) length(tmax) length(betabound) length(dec)]);

for i = length(ff):-1:1 %go backwards to preallocate entire structure
    
    pa.models(i).params = {beta{ff(i,1)} loc{ff(i,2)} yint{ff(i,3)} tmax{ff(i,4)}};
    pa.models(i).B = ones(1,length(pa.locs)+1);
    pa.models(i).Blocs = [num2cell(pa.locs) [pa.dectime]];
    pa.models(i).Bbounds = bbounds{ff(i,5)};
    pa.models(i).Btypes = [repmat({'stick'},1,length(pa.locs)) dec{ff(i,6)}];
    pa.models(i).Blocbounds = [bloclb blocub];
