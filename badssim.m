...
for i = 1:size(uBlocs0,1);
    uBlocs0(i,1:4) = sort(uBlocs0(i,1:4));
end
...
for i = 1:size(uinB,1)
    ...
    for j = 1:size(uB0,1)
        ...
        fprintf('Input %d, starting set %d\n',i,j)
        uBlocs0{j,5}(2) = uinBlocs(i,6);
        [uoutB(i,:,j), tempBloc, uouttmax(i,1,j), uoutyint(i,1,j), ucost(i,1,j), Ycalc,~,~,sflag] = glm_bads(Ymeas,window2,uB0(j,:),uBlocs0(j,:),Blocbounds,Btypes,Blabels,Bbounds,utmax0(j),tmaxbounds,uyint0(j),modelparams,'max');
